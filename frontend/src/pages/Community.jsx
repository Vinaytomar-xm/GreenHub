...
export default function Community() {
  ...
  const [modal, setModal]             = useState(null); // community object
  ...
  const openJoin = (c) => {
    if (!c.active) return;
    if (!user) { notify('Community join karne ke liye pehle login karo! 🔐', 'error'); navigate('/login?redirect=' + location.pathname); return; }
  ...
  const handleJoinSubmit = async () => {
    ...
    try {
      ...
      notify(err.response?.data?.message || 'Could not join. Try again.', 'error');
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ marginBottom:'2rem' }}>
        ...
        <h1 style={{ fontSize:'2rem', fontWeight:900, letterSpacing:'-1px', marginBottom:'.5rem' }}>Power Your Community</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:'1rem' }}>
        {[
          { icon:'📊', title:'Impact Dashboard', desc:"Track your community contribution to India's renewable energy goals with live data." },
          { icon:'🎓', title:'Green Education Hub', desc:'Learn about renewable tech, energy trading, and sustainability practices.' },
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.75)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--green-border)', borderRadius:'18px', padding:'2rem', width:'100%', maxWidth:'480px', boxShadow:'0 20px 60px rgba(0,0,0,.5)' }}>
            ...
            <div style={{ display:'flex', gap:'.75rem', marginTop:'1.5rem' }}>
              <button onClick={() => setModal(null)} className="btn-secondary" style={{ flex:1, justifyContent:'center' }}>Cancel</button>
              <button onClick={handleJoinSubmit} disabled={submitting} className="btn-primary" style={{ flex:2, justifyContent:'center' }}>
