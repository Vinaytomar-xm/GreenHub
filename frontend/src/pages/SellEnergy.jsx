...
const STEPS = ['Aapki Info','Energy Details','Pricing','Review & Submit'];
const BLANK = { ownerName:'',email:'',phone:'',location:'',ownerType:'Individual (Ghar wala)',name:'',type:'Solar',description:'',capacity:'',capacityUnit:'kW',available:'',price:'' };
...
export default function SellEnergy() {
  ...
  const submit = async () => {
    if (!user) { notify('Energy sell karne ke liye pehle login karo! 🔐', 'error'); navigate('/login?redirect=' + location.pathname); return; }
    setLoading(true);
    try {
      const res = await createProducer({ ...form, capacity: parseFloat(form.capacity), available: parseFloat(form.available || form.capacity), price: parseFloat(form.price) });
      setDone(res.data);
      notify(res.data.message);
    } catch (e) {
      notify(e.response?.data?.message || 'Registration failed. Is the backend running on port 5000?', 'error');
  ...
  if (done) return (
    <div className="fade-in" style={{ textAlign: 'center', padding: '5rem 2rem', maxWidth: '540px', margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--muted)', lineHeight: 1.7, marginBottom: '2rem' }}>
        <strong style={{ color: 'var(--text)' }}>{form.name}</strong> ka listing submit ho gaya!<br />
        Hamaari team 24 hours mein verify karegi aur <strong style={{ color: 'var(--text)' }}>{form.email}</strong> pe contact karegi.
      </p>
      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        <button className="btn-primary" onClick={() => navigate('/marketplace')}>Marketplace Dekho →</button>
  ...
  return (
    <div className="fade-in" style={{ maxWidth: '680px', margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontSize: '2rem', fontWeight: 900, letterSpacing: '-1px', marginBottom: '.5rem' }}>Apni Energy Becho</h1>
        <p style={{ color: 'var(--muted)' }}>Apna renewable energy source register karo aur seedha consumers se connect ho</p>
      </div>
      ...
      <div className="card" style={{ padding: '2rem' }}>
        ...
        {step === 0 && <>
          <h3 style={SH}>👤 Aapki Personal Information</h3>
          <FL label="Aapka Poora Naam *"><input style={IN} value={form.ownerName} onChange={e => set('ownerName', e.target.value)} placeholder="e.g. Ramesh Kumar" /></FL>
          <FL label="Email Address *"><input style={IN} type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="aap@example.com" /></FL>
          <FL label="Phone Number *"><input style={IN} value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+91 98765 43210" /></FL>
          <FL label="Sheher / State *"><input style={IN} value={form.location} onChange={e => set('location', e.target.value)} placeholder="e.g. Jaipur, Rajasthan" /></FL>
          <FL label="Aap Kaun Hain">
            <select style={IN} value={form.ownerType} onChange={e => set('ownerType', e.target.value)}>
              {['Individual (Ghar wala)','Kisan (Farmer)','Business Owner','NGO','Government'].map(t => <option key={t}>{t}</option>)}
            </select>
          </FL>
        </>}
        ...
        {step === 1 && <>
          ...
          <FL label="Available Capacity (khali chhoodo = same as capacity)">
          ...
          <FL label="Description *">
            <textarea style={{ ...IN, height: '80px', resize: 'vertical' }} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Apne setup ke baare mein batao — e.g. '5 kW rooftop solar, 2022 mein lagaaya, Jaipur suburb'" />
          </FL>
        </>}

        {/* STEP 2 — Pricing */}
        {step === 2 && <>
          <h3 style={SH}>₹ Price Set Karo</h3>
          ...
          {form.price && parseFloat(form.price) > 0 && (
            <div style={{ padding: '1rem', background: 'var(--green-dim)', border: '1px solid var(--green-border)', borderRadius: '8px', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '.5rem', textAlign: 'center', fontSize: '.82rem' }}>
                {[['Grid Rate','₹6–8/kWh','var(--dim)'],['Market Avg','₹3.5–5/kWh','var(--muted)'],['Aapka Price',`₹${form.price}/kWh`,'var(--green)']].map(([l,v,c]) => (
              ...
              {form.capacity && <p style={{ marginTop: '.75rem', fontSize: '.82rem', color: 'var(--muted)' }}>Agar fully sell karo: <strong style={{ color: 'var(--yellow)' }}>₹{(parseFloat(form.price)*parseFloat(form.capacity)).toFixed(0)}/month potential</strong></p>}
            </div>
          )}
        </>}
        ...
        {step === 3 && <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem', marginBottom: '1.5rem' }}>
            {[['Owner',form.ownerName],['Email',form.email],['Phone',form.phone],['Location',form.location],['Listing Name',form.name],['Energy Type',form.type],['Capacity',`${form.capacity} ${form.capacityUnit}`],['Available',`${form.available||form.capacity} ${form.capacityUnit}`],['Price',`₹${form.price}/kWh`]].filter(([,v])=>v).map(([l,v]) => (
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '.6rem .85rem', background: 'rgba(255,255,255,.02)', borderRadius: '7px', fontSize: '.88rem' }}>
          ...
          <div style={{ padding: '.9rem 1rem', background: 'rgba(251,191,36,.06)', border: '1px solid rgba(251,191,36,.2)', borderRadius: '8px', fontSize: '.83rem', color: 'var(--muted)', lineHeight: 1.6 }}>
            ⚠️ Submit karne ke baad hamaari team 24 hours mein verify karegi. <strong style={{ color: 'var(--text)' }}>{form.email}</strong> pe email aayegi.
          </div>
        </>}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2rem', gap: '1rem' }}>
          {step > 0
            ? <button className="btn-secondary" onClick={() => setStep(s => s - 1)}>← Wapas</button>
            : <button className="btn-secondary" onClick={() => navigate('/')}>← Home</button>}
          {step < 3
            ? <button className="btn-primary" disabled={!canNext[step]()} onClick={() => setStep(s => s + 1)}>Aage Badho →</button>
            : <button className="btn-primary" disabled={loading} onClick={submit}>{loading ? '⏳ Submitting...' : '🚀 Listing Submit Karo'}</button>}
        </div>
      </div>
    </div>
  );
