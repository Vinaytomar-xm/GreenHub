    ...
    <NotificationProvider>
      ...
      <footer style={{ position:'relative', zIndex:1, borderTop:'1px solid var(--border)', padding:'2rem 1.5rem' }}>
        <div style={{ maxWidth:'1400px', margin:'0 auto', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'1.5rem' }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:'.5rem', marginBottom:'.5rem' }}>
              ...
              <strong style={{ color:'var(--green)', fontSize:'1.05rem' }}>GreenGrid</strong>
            </div>
            <p style={{ color:'var(--dim)', fontSize:'.82rem', maxWidth:'260px', lineHeight:1.6 }}>India's decentralized renewable energy marketplace — buy, sell &amp; invest in green energy.</p>
          </div>
          <div style={{ display:'flex', gap:'3rem', flexWrap:'wrap' }}>
            {[
              { title:'Aapke Liye', links:[['🔆 Sell Energy','/sell-energy'],['🛒 Buy Energy','/buy-energy'],['📈 Invest','/connect?role=investor']] },
              ...
              { title:'Support',    links:[['💬 Ask Query / Help','/support'],['Connect','/connect'],['Login','/login'],['Sign Up','/signup']] },
            ].map(col => (
              <div key={col.title}>
                ...
                {col.links.map(([label, href]) => (
                  <a key={label} href={href} style={{ display:'block', color:'var(--dim)', fontSize:'.85rem', marginBottom:'.4rem', textDecoration:'none' }}
        ...
        <div style={{ maxWidth:'1400px', margin:'1.5rem auto 0', paddingTop:'1.5rem', borderTop:'1px solid var(--border)', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'.5rem' }}>
          <p style={{ color:'var(--dim)', fontSize:'.78rem' }}>© 2025 GreenGrid — Renewable Energy Marketplace Portal · Problem Statement 2</p>
          <p style={{ color:'var(--dim)', fontSize:'.78rem' }}>Promote Decentralized Energy Exchange · P2P Green Energy Trading · Community Sustainability</p>
        </div>
      </footer>
    </NotificationProvider>
  );
